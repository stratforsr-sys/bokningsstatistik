...
export class GraphService {
  private apiUrl = config.graph.apiUrl;
  private authUrl = config.graph.authUrl;
  ...
  getAuthUrl(state?: string): string {
    ...
    return `${this.authUrl}/${config.azure.tenantId}/oauth2/v2.0/authorize?${params.toString()}`;
  }
  ...
  async getTokenFromCode(code: string): Promise<TokenResponse> {
    try {
      ...
      const response = await axios.post<TokenResponse>(
        `${this.authUrl}/${config.azure.tenantId}/oauth2/v2.0/token`,
        params.toString(),
        {
          headers: {
